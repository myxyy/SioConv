...
def main(cfg):
    ...
    def predict(prompt):
        ...
        hidden_last = torch.randn(1,128,model.dim).cuda()
        while prompt_len - current_len > context_len:
            _, hidden = model(prompt[current_len:current_len+context_len].view(1,context_len), hidden_last)
            hidden_last = hidden[:,:,-1,:]
            current_len += context_len
            start += context_len
        model.set_is_refresh(False)

        predict_init, hidden = model(prompt[current_len:current_len+context_len].view(1,context_len), hidden_last)
        _, predict_init_i = predict_init.view(context_len, vocab_size)[prompt_len - current_len -1].topk(beam_width)
        hidden_last = hidden_last.repeat(beam_width,1,1)
        ...
        while current_len < length:
            ...
            predict_beam, hidden = model(prompt_beam[:,start:start+context_len], hidden_last)
            ...
            hidden_last = hidden_last[torch.div(predict_beam_i, vocab_size, rounding_mode='floor')]
            ...
            if current_len % context_len == 1 or context_len == 1:
                hidden_last = hidden[torch.div(predict_beam_i, vocab_size, rounding_mode='floor'),:,-1,:]
