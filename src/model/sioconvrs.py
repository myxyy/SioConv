...
class SioConvRSLayer(nn.Module):
    def __init__(self, dim: int):
        ...
        self.fc_output = nn.Linear(dim, dim)
    ...
    def forward(self, x):
        ...
        y = self.fc_output(h) * self.act(self.fc_output_gate(x))
        return y
...
class SioConvRSBlock(nn.Module):
...
class SioConvRS(nn.Module):
    def __init__(
        ...
        self.block_list = nn.ModuleList([SioConvRSBlock(dim, dim_ff_hidden, dropout) for _ in range(depth)])
